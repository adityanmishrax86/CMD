...
@click.option('-au', '--audience', type=str, help='Audience URL', required=True)
def login(username, auth_url, audience):
    '''
        Login to Management Site.
        Login using registered Email Address, Auth URL, password, Client ID and Client Secret. 
    '''
...
def get(url):
    ...
    try:
        ...
        if res.status_code == 401:
            ...
            click.echo("Route does not exist. Please check the Route!!")
            return
        click.echo(res.request)
        click.echo(res.reason)
        print(res.text)

    except JSONDecodeError as e:
        click.echo(
            "There are some error while getting from context, Relogin to fix the Issue!")
    except OSError as e:
        click.echo(
